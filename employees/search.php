...
if(isset($_GET['search'])){
    ...
    $select="SELECT * FROM `employeeswithdepartement` WHERE empname LIKE '%$namevalue%'";
    $s=mysqli_query($connection,$select);
    }

    if(isset($_GET['searchbyDESC'])){
        $namevalue=$_GET['namevalue'];
        $select="SELECT * FROM `employeeswithdepartement` ORDER BY `empname` DESC";
        $s=mysqli_query($connection,$select);
        }
        if(isset($_GET['searchbyASC'])){
            $namevalue=$_GET['namevalue'];
            $select="SELECT * FROM `employeeswithdepartement` ORDER BY `empname` ASC";
            $s=mysqli_query($connection,$select);
            }
    ...
    <form class="form" action="./search.php" method="get">
        <div class="row my-3">
                    <div class="col-md-6">
                        ...
                        <input id="myInput" name="namevalue" class="form-control" type="text" placeholder="search">
                        </div>
                    </div>
                    <div class="col-md-2">                       
